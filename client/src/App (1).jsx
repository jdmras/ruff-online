...
function SeatBidBadge({ text, className = "" }) {
  if (!text) return null;

  return (
    <div className={`seat-bid-badge ${className}`.trim()}>
      {text}
    </div>
  );
}
...
export default function App() {
  ...
  const showSeatBidBadges =
    !!state && (state.phase === "BID" || state.phase === "DECLARE");

  const latestBidBySeat = useMemo(() => {
    const out = {};

    if (!state?.bids?.length) return out;

    for (const b of state.bids) {
      if (b.type === "PASS") {
        out[b.seat] = "P";
      } else if (b.type === "BID") {
        out[b.seat] = String(b.amount);
      } else if (b.type === "TAKE") {
        out[b.seat] = `T${b.amount ? ` ${b.amount}` : ""}`;
      }
    }
  ...
  const seatBidTextForPos = (pos) => {
    const seat = relativeSeatMap?.[pos];
    if (seat === undefined || seat === null) return "";
    return latestBidBySeat[seat] ?? "";
  };
        ...
        <div className="table-felt">
          ...
          {playersByPos && relativeSeatMap ? (
            <>
              ...
              {showSeatBidBadges && (
                <SeatBidBadge
                  text={seatBidTextForPos("top")}
                  className="seat-bid-top"
                />
              )}
              ...
              {showSeatBidBadges && (
                <SeatBidBadge
                  text={seatBidTextForPos("left")}
                  className="seat-bid-left"
                />
              )}
              ...
              {showSeatBidBadges && (
                <SeatBidBadge
                  text={seatBidTextForPos("right")}
                  className="seat-bid-right"
                />
              )}
              ...
              {showSeatBidBadges && (
                <SeatBidBadge
                  text={seatBidTextForPos("bottom")}
                  className="seat-bid-bottom"
                />
              )}
